...
| プラットフォーム | 字幕 | ステータス |
|------|------|------|
| **YouTube** | Manual CC (not auto-generated) — primary `/api/timedtext` + transcript-panel fallback | テスト済み |
| **Netflix** | TTML via Cadmium player API | テスト済み |
| **HBO Max** | WebVTT via DASH manifest | テスト済み |
| **BBC iPlayer** | TTML/EBU-TT-D | テスト済み |
| **RaiPlay** | SRT via MAIN-world fetch interceptor | テスト済み |
| **kino.pub** | HLS subtitles (Vidstack player) | テスト済み |
| **RTS Planeta** | Native HTML5 `<track>` | テスト済み |
| **ARTE** | HLS subtitles | テスト済み |
| **Plex** | HLS subtitles | テスト済み |
| **Filmzie** | HLS subtitles | テスト済み |

HLS / VTT / TTML / ネイティブ `<track>` 字幕を提供する他のサイトも動作する可能性があります — 拡張機能のポップアップで **Enable** をクリックしてください。
